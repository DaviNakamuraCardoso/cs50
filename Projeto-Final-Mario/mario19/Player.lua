...
function Player:init(map)
    ...
    self.cover = Block(self.map, 0, 0, VIRTUAL_WIDTH, VIRTUAL_HEIGHT, colors['black'])
    self.nextLevelButton = Button(self.map, VIRTUAL_WIDTH / 2 - 50, VIRTUAL_HEIGHT / 3, 100, 20, colors['yellow'])

    -- Messages

    self.message1 = Message(0, VIRTUAL_HEIGHT / 2, 'fonts/four.ttf', 14, colors['white'])
    self.message2 = Message(0, VIRTUAL_HEIGHT / 2 + 30, 'fonts/four.ttf', 14,
    colors['white'])
    ...
    self.behaviors = {
        ...
        ['winning'] = function(dt)
            ...
            elseif self.map:collides(self.map:tileAt(self.x, self.y + self.height)) or
                self.map:collides(self.map:tileAt(self.x + self.width - 1, self.y + self.height)) then
                if self.map.newLevel then
                    ...
                    self.dy = 0
                    self.y = (self.map:tileAt(self.x, self.y + self.height).y - 1)*    self.map.tileHeight - self.height + 1
...
function Player:update(dt)
    ...
    self.message1:update(dt)
    self.message2:update(dt)
...
function Player:render()
    ...
    if self.state == 'reviving' then
        self.message1:twinkle('PRESS ENTER TO REVIVE', 0.4, self.sounds['reviving'])
    end
    ...
    if self.map.newLevel then
        ...
        -- Messages
        self.message1:prettyShow(self.score, self.sounds['score'],'SCORE:', 2)
        self.message2:prettyShow(self.coins, self.sounds['box'],'BOXES: ', 1)
    end
end
