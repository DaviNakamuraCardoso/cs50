...
function Enemy:init(map, spritesheet, x)
    ...
    self.behaviors = {
        ['walking'] = function(dt)
            ...
            if self.map:tileAt(self.x - self.map.tileWidth, self.y + self.height).id == TILE_EMPTY and self.map:tileAt(self.x + self.map.tileWidth, self.y + self.height).id == TILE_EMPTY then
                self.state = 'stopped'
        ...
        ['fly'] = function(dt)
            if self.map:collides(self.map:tileAt(self.x, self.y  + self.height)) or self.map:collides(self.map:tileAt(self.x + self.width, self.y + self.height)) then
                if self.direction > 0 then
                    ...
                    self.y = math.floor(self.y + self.dy * self.direction * dt)
                end
            else
