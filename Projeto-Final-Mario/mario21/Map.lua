...
function Map:init(level)
    ...
    for i=1, self.numberOfEnemies do
        ...
        self.enemies[i] = Enemy(self, 'graphics/red_alien.png', enemyPosition)
    end
    ...
    while x < self.mapWidth-20 do
        ...
        elseif math.random(30) == 1 then
            local height = math.random(8)
            self:generatePiramid(20, self.mapHeight / 2, height, 'more')
            x = x + 2 * height
        elseif math.random(20) == 1 then
            ...
            -- 10% chance to generate bush, being sure to generate away froedge
        ...
        elseif math.random(10) ~= 1 then
            ...
            if math.random(15) == 1 then
                ...
                if math.random(2) == 1 then
                    self:setTile(x + 1, ground - 4, JUMP_BLOCK)
                elseif math.random(5) == 1 then
                    self:setTile(x + 1, ground - 4, JUMP_BLOCK)
                    self:setTile(x + 2, ground - 4, JUMP_BLOCK)
                end
...
function Map:enemyKilling()
    for i=1, self.numberOfEnemies do
        if self.player.x + self.player.width >= self.enemies[i].x and self.enemies[i].x +     self.enemies[i].width > self.player.x and self.player.y + self.player.height >   self.enemies[i].y and self.player.y < self.enemies[i].y + self.enemies[i].height then
